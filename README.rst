...
The base package includes a native Rust extension that accelerates reference
resolution:

* ``_prance_rs`` — a Rust/PyO3 resolver, parser primitives, OpenAPI
  validator (``validate_openapi_spec``, ``load_openapi_spec_py``,
  ``parse_and_validate_spec`` for the ``openapi-spec-validator`` backend), and
  full parse+resolve pipeline (preferred when built; requires a Rust compiler
  when installing from source)

The preferred backend chain is ``Rust -> pure Python``. Set
``PRANCE_BACKEND=rust`` or ``python`` to force a specific tier.
